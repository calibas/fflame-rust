...
**Location**: `assets/palettes/packs/*.json` — every JSON file in this folder is a pack (`builtin.json` also ships embedded in the binary as the offline fallback).

**Shipped Packs**:
- **Built-in** (embedded) - always available, enabled
- **Starter Pack** (12 palettes) - Enabled by default
- **Apophysis 1-4** (~700 palettes total) - Disabled by default; the classic Apophysis gradients
...
**Desktop Builds**:
```
Route 1: Embedded builtin pack (offline fallback, always first)
Route 2: Directory scan of assets/palettes/packs/*.json
         (skips builtin.json/manifest.json; all packs load eagerly;
          each pack's enabled_by_default decides list membership)
...
**WASM Builds**:
```
Route 1: Embedded builtin pack
Route 2: Embedded manifest (generated by build.rs from the packs
         folder) -> pack metadata; content lazily fetched over HTTP
         from assets/palettes/packs/ when a pack is enabled
