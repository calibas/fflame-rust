...
impl ShaderConstants {
    ...
    pub fn to_wgsl(&self) -> String {
        ...
        // An EMPTY flame takes the non-inlined path: `array<f32, 0>`
        // is not valid WGSL (element counts must be positive), and a
        // flame with no transforms is reachable -- an escape-time
        // config does not use the chaos game at all, so a script that
        // builds one leaves the flame empty and the shader still has
        // to compile.
        if let Some(ref cumulative) = self.cumulative_weights.as_ref().filter(|c| !c.is_empty()) {
        ...
        // Same empty-flame guard as the weights above: every inlined
        // array below would otherwise be zero-length.
        if let Some(ref transforms) = self.inlined_transforms.as_ref().filter(|t| !t.is_empty()) {
            code.push_str("\n// Inlined transform data (eliminates buffer reads)\n");
...
mod tests {
    ...
    /// A flame with NO transforms must still produce compilable WGSL.
    ///
    /// It is reachable: an escape-time config does not use the chaos
    /// game, so a script that builds one leaves the flame empty -- and
    /// the escape path still builds the flame shader for the palette
    /// and tone-map tail. The inlined arrays were emitted as
    /// `array<f32, 0>`, which naga rejects ("array element count must
    /// be positive"), so the very first escape config generated by a
    /// script failed to export.
    #[test]
    fn an_empty_flame_emits_valid_wgsl_arrays() {
        let constants = ShaderConstants {
            num_transforms: 0,
            cumulative_weights: Some(Vec::new()),
            inlined_transforms: Some(Vec::new()),
            ..Default::default()
        };
        let wgsl = constants.to_wgsl();
        assert!(
            !wgsl.contains(", 0>"),
            "a zero-length array reached the shader source:
{wgsl}"
        );
        assert!(wgsl.contains("const USE_INLINED_WEIGHTS: bool = false;"));
        assert!(wgsl.contains("const USE_INLINED_TRANSFORMS: bool = false;"));
    }
