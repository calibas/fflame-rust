...
pub fn render_settings_content(
    ...
    // Almost nothing in this panel survives a non-flame mode: the
    // chaos game is what it configures, and `should_iterate` is false
    // in Escape and Simulation (ui-render-modes plan, section 3.4).
    // Pause looks universal but is read in two places that both
    // already exclude those modes, so it is inert there too. What is
    // left is VSync and the frame cap, which are device preferences,
