...
pub fn render_levels_controls(
    ...
    ui.add_enabled_ui(!levels.auto_enabled, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.add(egui::Slider::new(&mut levels.input_black, range).logarithmic(true)).changed() {
                changed = true;
            }
        });
        ...
        ui.horizontal(|ui| {
            ui.label("White:");
            let range = 0.0..=histogram.max_density.max(100.0);
            if ui.add(egui::Slider::new(&mut levels.input_white, range).logarithmic(true)).changed() {
                changed = true;
            }
        });
    });
    ...
    ui.horizontal(|ui| {
        ...
        if ui.add(egui::Slider::new(&mut gamma, 0.1..=10.0).logarithmic(true)).changed() {
