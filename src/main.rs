...
fn main() {
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        match cli.command {
            ...
            Some(Commands::Export { input, output, width, height, category, iterations_per_thread, dump_shader, transparent }) => {
                // Run in headless export mode
                fractal_flame_wgpu::export_mode(&input, &output, width, height, category, iterations_per_thread, dump_shader, transparent);
            }
