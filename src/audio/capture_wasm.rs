...
impl AudioCapture {
    ...
    /// List available input devices (not fully supported in WASM).
    pub fn list_devices() -> Vec<String> {
        // In WASM, device enumeration requires permission first
        // and is more complex. Return empty list for now.
        vec![]
    }
    ...
    /// Start capturing (device_name is ignored in WASM).
    pub fn start_device(&mut self, _device_name: Option<&str>) -> Result<(), CaptureError> {
        ...
        // Create constraints for audio-only capture
        ...
        let success_callback = Closure::once(Box::new(move |stream: JsValue| {
            ...
            *context_holder.borrow_mut() = Some(WasmCaptureContext {
            ...
            log::info!("WASM audio capture started successfully");
