...
pub static KLEIN_GROUP: VariationDef = VariationDef {
    ...
    parameters: &[
        ...
        param!("avoid_reversal", "Avoid Reversal", bool, true, "When 1, the next-matrix pick excludes the inverse of the previous matrix (no `aA`, `Aa`, `bB`, or `Bb` cancellations), so the trajectory drifts through the limit set instead of bouncing back-and-forth on a small subset. When 0, all four matrices are equally likely each iteration."),
    ],
