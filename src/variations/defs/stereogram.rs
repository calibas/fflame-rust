...
pub static STEREOGRAM: VariationDef = VariationDef {
    ...
    parameters: &[
        ...
        param!("z_conv", "Convergence", unlimited_float, 1.0, 0.05, 10.0, "Camera-space depth of the zero-parallax plane — content at this depth sits exactly on the 'screen'; nearer pops out, farther recedes. Requires Perspective > 0 in the View panel (orthographic projection has no parallax, so the panels come out identical)."),
        param!("view", "View", enum, 0, &["Parallel", "Cross", "Triptych L-R-L"], "Panel arrangement. Parallel: left-eye image in the left panel (relaxed/wall-eyed viewing; keep panels small on screen). Cross: panels swapped for cross-eyed viewing — no size limit. Triptych L-R-L: three panels serving BOTH methods at once (view 1-2 parallel, 2-3 cross). If depth looks inverted (near reads as far), you are fusing with the other method — switch modes."),
