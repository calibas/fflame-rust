...
pub static QUATERNION_CUBIC: VariationDef = VariationDef {
    ...
    parameters: &[
        ...
        param!("w_color", "Color by W", float, 0.0, 0.0, 8.0, "0 = off. >0 = write a palette index from the 4th coordinate (3D: fract(w * scale); 2D: fract(|z| * scale)) — reveals the hidden dimension as COLOR without altering the attractor. Needs the transform's direct_color > 0."),
    ],
    wgsl_2d: WGSL_2D,
    wgsl_3d: WGSL_3D,
};
...
const WGSL_2D: &str = r#"
fn variation_quaternion_cubic(p: vec2<f32>, xform_id: u32, variation_id: u32, rng: ptr<function, RngState>, vc: ptr<function, f32>) -> vec2<f32> {
    ...
    if (get_param(xform_id, variation_id, 5u) > 0.5) {
        ...
        out = pow(length(d), 1.0 / 3.0) * vec2<f32>(cos(theta), sin(theta));
    } else {
        // Forward: z^3 + c.  z^3 = (x^3 - 3xy^2,  3x^2 y - y^3).
        let x = p.x;
        let y = p.y;
        out = vec2<f32>(x * x * x - 3.0 * x * y * y + cx, 3.0 * x * x * y - y * y * y + cy);
    }
...
fn variation_quaternion_cubic(p: vec3<f32>, xform_id: u32, variation_id: u32, rng: ptr<function, RngState>, vc: ptr<function, f32>) -> vec3<f32> {
