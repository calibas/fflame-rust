...
impl App {
    ...
    pub fn import_config(&mut self, config: FractalConfig) {
        ...
        if let Some(ref mut renderer) = self.flame_renderer {
            ...
            if let Some(palette) = config.palette.as_ref().or_else(|| self.palette_library.get(config.palette_index)) {
                renderer.load_config(&self.gpu.device, &mut encoder, &self.gpu.queue, &config, palette, self.system_settings.iterations_per_thread);
            }
    ...
    pub fn export_custom_size(&self, transparent: bool, config: FractalConfig, _render_time_ms: f64) {
        ...
        println!("  iterations_per_thread: {}", self.system_settings.iterations_per_thread);
        ...
        temp_renderer.load_config(&self.gpu.device, &mut encoder, &self.gpu.queue, &config, palette, self.system_settings.iterations_per_thread);
        self.gpu.queue.submit(std::iter::once(encoder.finish()));
        ...
        while total_rendered < target {
            ...
            temp_renderer.compute_pass(
                ...
                &self.gpu.queue,
                NUM_WORKGROUPS,
                self.system_settings.iterations_per_thread, // CHANGED: Use full value like viewport
                config.zoom,
                config.pan_x,
                config.pan_y,
                config.rotation,
            ...
            let samples_this_frame = NUM_WORKGROUPS as u64 * THREADS_PER_WORKGROUP * self.system_settings.iterations_per_thread as u64;
