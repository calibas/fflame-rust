    ...
    pub(super) fn handle_mouse_move(&mut self, x: f32, y: f32, consumed: bool) {
        if self.mouse_dragging {
            ...
            if let Some((last_x, last_y)) = self.last_mouse_pos {
                ...
                let pan_dx = -dx / (scale * config.zoom);
                let pan_dy = -dy / (scale * config.zoom); // Negative to match drag direction
                ...
                let new_pan_x = config.pan_x + rotated_dx;
                let new_pan_y = config.pan_y + rotated_dy;
                let _ = self.config_manager.update_batch(
                    vec![
                        (crate::config::ConfigPath::PanX, new_pan_x.into()),
                        (crate::config::ConfigPath::PanY, new_pan_y.into()),
        ...
        if zoom_factor != 1.0 {
            ...
            if zoom_factor > 1.0 {
                ...
                if let Some((mouse_x, mouse_y)) = self.last_mouse_pos {
                    ...
                    let new_zoom = config.zoom * zoom_factor;
