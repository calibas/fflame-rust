...
impl App {
    ...
    fn render(&mut self, window: &Window) -> Result<(), SurfaceError> {
        ...
        if !preset_loaded {
            ...
            // Only capture on drag START or slider changes, not during continuous dragging
            ...
            let should_capture = ui_response.triangle_drag_started || view_changed
                ...
                || (ui_response.flame_changed && !ui_response.triangle_dragging); // Capture slider changes, but not during triangle drag
            if should_capture {
                self.capture_state();
            }
        }
        ...
        if needs_update && !preset_loaded {
            if let Some(ref mut renderer) = self.flame_renderer {
                ...
                // For triangle dragging: reset on EVERY frame during drag to avoid smearing (accumulation over old image)
                ...
                let should_reset = ui_response.reset_requested || view_changed || ui_response.palette_changed || ui_response.color_mode_changed
                    ...
                    || ui_response.flame_changed;  // Always reset on flame changes (including during triangle drag)
                if should_reset {
