...
fn apply_variations(xform: Transform, xform_id: u32, p: vec2<f32>, rng: ptr<function, RngState>, hide: ptr<function, bool>) -> vec2<f32> {
    ...
    // Phase 4: Post-variations (modify output)

    // 1: Spray Blur (POST)
    if (xform.variations[1] != 0.0) {
        result = variation_spray_blur(result, xform_id, 1u, rng);
    }
...
fn get_param(xform_id: u32, variation_id: u32, param_slot: u32) -> f32 {
    ...
    switch (variation_id) {
        ...
        case 1u: { offset = 0u; }  // spray_blur: 6 slots
        default: { offset = 0u; }
    }
...
// Multi-emit collector (Feature::PlotEmits): up to 16 extra
             ...
             var<private> plot_emit_points: array<vec4<f32>, 16u>;
             ...
             fn emit_plot_weighted(p: vec2<f32>, w: f32) {
                 if (plot_emit_count < 16u) {
                     plot_emit_points[plot_emit_count] = vec4<f32>(p, 0.0, w);
                     plot_emit_count = plot_emit_count + 1u;
                 }
             }

             fn emit_plot(p: vec2<f32>) {
                 emit_plot_weighted(p, 1.0);
             }

             fn emit_plot_offset(p: vec2<f32>, w: f32) {
