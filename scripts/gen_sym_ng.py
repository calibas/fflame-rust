...
def emit_def(num, name, params, body2d, body3d, pretty, java_ref, kind):
    const = name.upper()
    plist = []
    for p, default in params:
        ...
        plist.append(
            ...
            f'-100.0, 100.0, "{kind}-symmetry parameter (see JWildfire {java_ref})."),'
        )
...
# (java_class, name_prefix, count, out_file, pretty_prefix, header_title, kind)
FAMILIES = [
    ("SymNetG", "sym_ng", 17, os.path.join("src", "variations", "defs", "sym_ng.rs"),
     "NG", "Network Symmetry Group variations sym_ng1..sym_ng17", "Network"),
    ("SymBandG", "sym_bg", 7, os.path.join("src", "variations", "defs", "sym_bg.rs"),
     "BG", "Band Symmetry Group variations sym_bg1..sym_bg7", "Band"),
]


def gen_family(java_class, name_prefix, count, out, pretty_prefix, title, kind):
    defs, consts = [], []
    for num in range(1, count + 1):
        name = f"{name_prefix}{num}"
        src = read(os.path.join(SRC, f"{java_class}{num}Func.java"))
        ...
        defs.append(emit_def(num, name, params, b2, b3,
                             f"Sym {pretty_prefix}{num}", f"{java_class}{num}", kind))
        consts.append(name.upper())
        print(f"  {name}: {len(mats)} mats, {len(params)} params"
              + (", offset" if offset else ""))

    header = f'''//! {title} (Jesus Sosa).
//!
//! GENERATED by scripts/gen_sym_ng.py from
//! `output/variation-jwf-source/{java_class}*Func.java`. Do not edit by hand —
